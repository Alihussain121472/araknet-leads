...
export const LeadsTableSection: React.FC<LeadsTableSectionProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl bg-card border border-border-default backdrop-blur-md shadow-sm dark:shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="form-control w-full rounded-xl pl-10 pr-4 py-2 text-xs text-text-primary placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
            />
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-3 border-t border-border-default">
          ...
          <select
            ...
            className="form-control rounded-lg px-2.5 py-1.5 text-xs text-text-primary focus:outline-none focus:border-blue-500"
          >
      ...
      {/* Main Leads Table */}
      <div className="rounded-2xl bg-card border border-border-default backdrop-blur-md shadow-sm dark:shadow-xl overflow-hidden">
              ...
              <tr className="border-b border-border-default bg-page text-[11px] font-bold text-text-secondary uppercase tracking-wider">
                <th className="p-4 w-10 text-center">
                  <button onClick={toggleSelectAll} className="text-text-secondary hover:text-text-primary">
                    {selectedLeadIds.length === paginatedLeads.length && paginatedLeads.length > 0 ? (
                      <CheckSquare className="w-4 h-4 text-brand-link" />
                    ) : (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    <ArrowUpDown className="w-3 h-3 text-accent-amber" />
                  </div>
                </th>
                ...
                <tr>
                  <td colSpan={9} className="p-8 text-center text-text-secondary">
                    <div className="flex items-center justify-center gap-2">
                      <RefreshCw className="w-4 h-4 animate-spin text-brand-link" />
                ...
                paginatedLeads.map((lead) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-card-hover transition-colors cursor-pointer group ${
                    ...
                    >
                      ...
                      {/* Business Name & Phone */}
                      <td className="p-4 font-medium text-text-primary group-hover:text-brand-link transition-colors">
                      ...
                      {/* Industry */}
                      <td className="p-4 text-text-primary">
                        <span className="px-2 py-1 rounded-md bg-card-hover border border-border-default text-[11px]">
                          {lead.industry}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <div className="w-10 h-10 rounded-xl bg-page border border-border-default flex items-center justify-center font-black text-sm text-accent-amber shadow-inner">
                            {lead.pitch_score}/10
                          </div>
                          <div className="text-[10px] text-text-secondary">
                            {lead.pitch_score >= 8 ? (
                              <span className="text-accent-amber font-bold block">Prime Deal</span>
                            ) : lead.pitch_score >= 5 ? (
                              <span className="text-brand-link font-medium block">High Upside</span>
                            ) : (
