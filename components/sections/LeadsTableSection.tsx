...
export const LeadsTableSection: React.FC<LeadsTableSectionProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 backdrop-blur-md shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950/80 border border-slate-300 dark:border-slate-700/80 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-800 dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700/80 border border-slate-300 dark:border-slate-700/60 text-xs font-semibold text-slate-800 dark:text-slate-200 transition-all shadow-sm"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-3 border-t border-slate-200 dark:border-slate-800/60">
          {/* Country Filter */}
          <select
            value={selectedCountry}
            onChange={(e) => { setSelectedCountry(e.target.value); setCurrentPage(1); }}
            className="bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-600 dark:text-slate-300 focus:outline-none focus:border-blue-500"
          >
      ...
      {/* Main Leads Table */}
      <div className="rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 backdrop-blur-md shadow-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                <th className="p-4 w-10 text-center">
                  <button onClick={toggleSelectAll} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">
                ...
                <th 
                  ...
                  className="p-4 cursor-pointer hover:text-slate-800 dark:text-slate-200 transition-colors"
                >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-center" onClick={(e) => toggleSelectLead(lead.id, e)}>
                        <button className="text-slate-500 hover:text-slate-600 dark:text-slate-300">
                          {isSelected ? (
                            <CheckSquare className="w-4 h-4 text-blue-400" />
                          ) : (
                            <Square className="w-4 h-4" />
                      ...
                      {/* Business Name & Phone */}
                      <td className="p-4 font-medium text-slate-900 dark:text-slate-100 group-hover:text-blue-400 transition-colors">
                        <div>{lead.business_name}</div>
                        {lead.phone && (
                          <div className="text-[11px] text-slate-500 dark:text-slate-400 font-normal mt-0.5">{lead.phone}</div>
                        )}
                      </td>
                      ...
                      <td className="p-4 text-slate-600 dark:text-slate-300">
                        <div>{lead.city}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400">{lead.country}</div>
                      </td>

                      {/* Industry */}
                      <td className="p-4 text-slate-600 dark:text-slate-300">
                        <span className="px-2 py-1 rounded-md bg-slate-800/80 border border-slate-300 dark:border-slate-700/60 text-[11px]">
                          {lead.industry}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-center font-black text-sm text-amber-400 shadow-inner">
                            {lead.pitch_score}/10
                          </div>
                          <div className="text-[10px] text-slate-500 dark:text-slate-400">
                            {lead.pitch_score >= 8 ? (
                              ...
                              <span className="text-slate-500 dark:text-slate-400 block">Moderate</span>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4" onClick={(e) => e.stopPropagation()}>
                        ...
                        >
                          <option value="new" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">New</option>
                          <option value="contacted" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Contacted</option>
                          <option value="proposal_sent" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Proposal Sent</option>
                          <option value="won" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Won</option>
                          <option value="lost" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">Lost</option>
                        </select>
                      </td>
        ...
        {/* Pagination Bar */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/40 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <div>
            Showing <span className="text-slate-800 dark:text-slate-200 font-semibold">{filteredLeads.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}</span> to{' '}
            <span className="text-slate-800 dark:text-slate-200 font-semibold">
              {Math.min(currentPage * itemsPerPage, filteredLeads.length)}
            </span>{' '}
            of <span className="text-slate-800 dark:text-slate-200 font-semibold">{filteredLeads.length}</span> leads
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed text-slate-800 dark:text-slate-200 font-medium transition-colors"
            >
              Previous
            </button>
