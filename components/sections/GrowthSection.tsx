...
export function GrowthSection({ leads, busy, onBusyChange, onRefresh, onOpenLead }: { leads: Lead[]; busy: boolean; onBusyChange: (busy: boolean) => void; onRefresh: () => Promise<void>; onOpenLead: (lead: Lead) => void }) {
  ...
  const panel = 'rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 space-y-4';
  const field = 'w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-3 text-sm';
  return <div className="space-y-6">
    ...
    <section className={panel}>
      ...
      {health && <div className="flex flex-wrap gap-4 text-xs text-slate-500 dark:text-slate-400"><span>Directory: {health.directory}</span><span>Scheduler: {health.cronConfigured ? health.scheduleEnabled ? 'Enabled' : 'Ready; enable in Agent Control' : 'Needs configuration'}</span></div>}
    </section>
    <section className={panel}>
      <h2 className="text-lg font-bold">Search several cities</h2>
      <p className="text-sm text-slate-500 dark:text-slate-400">Up to five cities, searched one at a time. Keep this page open. Results are saved after each city. Configured paid directories may charge for each search.</p>
      ...
      <div className="flex flex-wrap items-center gap-4"><button onClick={runCampaign} disabled={busy || !health?.databaseReady || !country.trim() || !cities.length || cities.length > 5} className="rounded-lg bg-blue-600 px-5 py-3 text-sm font-semibold disabled:opacity-40">{campaign ? 'Searching...' : `Search ${cities.length} cities`}</button><span className="text-xs text-slate-500 dark:text-slate-400">Up to {Math.min(cities.length,5)*batch} results before deduplication</span>{campaign && <button onClick={()=>{stop.current=true;setProgress(lines=>[...lines,'Stop requested; current city will finish.']);}} className="text-sm text-amber-300">Stop after this city</button>}</div>
      {cities.length > 5 && <p role="alert" className="text-sm text-amber-300">Use five cities or fewer per campaign.</p>}
      <div role="log" aria-live="polite" className="space-y-1 text-sm text-slate-500 dark:text-slate-400">{progress.map((line,i)=><p key={i}>{line}</p>)}</div>
    </section>
    <div className="grid gap-6 xl:grid-cols-2">
      <section className={panel}>
        <h2 className="text-lg font-bold">Prioritized shortlist</h2><p className="text-sm text-slate-500 dark:text-slate-400">Ranks website opportunity, industry fit, contact availability and review evidence. This is a prospecting score, not a conversion prediction.</p>
        <div className="flex flex-wrap gap-4 text-sm"><label>Minimum priority <select className="bg-slate-50 dark:bg-slate-950 p-2 rounded" value={minScore} onChange={e=>setMinScore(Number(e.target.value))}>{[0,40,60,80].map(n=><option key={n}>{n}</option>)}</select></label><label><input type="checkbox" checked={phoneOnly} onChange={e=>setPhoneOnly(e.target.checked)} /> Phone listed</label><label><input type="checkbox" checked={newOnly} onChange={e=>setNewOnly(e.target.checked)} /> New leads only</label></div>
        <p className="text-xs text-slate-500">{ranked.length} matching leads</p>
        <div className="max-h-[560px] overflow-y-auto space-y-2">{ranked.slice(0,100).map(({lead,insight})=><button key={lead.id} onClick={()=>selectLead(lead)} className={`w-full rounded-xl border p-4 text-left ${selectedId===lead.id?'border-blue-500 bg-blue-950/40':'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50'}`}><div className="flex justify-between gap-3"><strong className="text-sm">{lead.business_name}</strong><span className="text-cyan-400 font-bold">{insight.priority}/100</span></div><p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{lead.city} · {lead.industry}</p><p className="text-xs text-slate-500 dark:text-slate-400 mt-2">{insight.nextAction}</p></button>)}</div>
        {!ranked.length && <p className="text-sm text-slate-500 dark:text-slate-400">No matching leads yet. Complete setup, run a search, or loosen your filters.</p>}
      </section>
      <section className={panel}>
        <h2 className="text-lg font-bold">Evidence and outreach draft</h2>
        {selected && insight ? <><h3 className="font-semibold">{selected.business_name}</h3><dl className="text-sm space-y-2">{Object.entries(insight.breakdown).map(([key,value])=><div key={key} className="flex justify-between"><dt className="text-slate-500 dark:text-slate-400">{key}</dt><dd>{value} points</dd></div>)}</dl><p className="text-xs text-slate-500 dark:text-slate-400">Data completeness: {insight.completeness}% · Source: {selected.source_provider} · Last updated: {insight.age ?? 'unknown'} days ago. A missing field is not proof that a business lacks that capability.</p><p className="text-sm text-cyan-300">Next step: {insight.nextAction}</p><label className="block text-sm">Editable draft<textarea rows={10} value={draft} onChange={e=>{setDraft(e.target.value);setCopied(false);}} className={field} /></label><p className="text-xs text-slate-500">Template based on listing data. Verify claims, add your identity and review before sending. Nothing is sent automatically.</p><div className="flex gap-4"><button className="rounded-lg bg-blue-600 px-4 py-2 text-sm" onClick={async()=>{try{await navigator.clipboard.writeText(draft);setCopied(true);}catch{setError('Clipboard unavailable. Select and copy the draft manually.');}}}>{copied?'Copied':'Copy draft'}</button><button className="text-sm text-blue-400" onClick={()=>onOpenLead(selected)}>Open full lead</button></div></> : <p className="text-sm text-slate-500 dark:text-slate-400">Choose a lead to see its score breakdown, next step and personalized draft.</p>}
      </section>
    </div>
  </div>;
}
