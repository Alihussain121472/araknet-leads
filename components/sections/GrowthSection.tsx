...
export function GrowthSection({ leads, busy, onBusyChange, onRefresh, onOpenLead }: { leads: Lead[]; busy: boolean; onBusyChange: (busy: boolean) => void; onRefresh: () => Promise<void>; onOpenLead: (lead: Lead) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <section className={panel}>
      ...
      <p className="text-sm text-text-secondary">Up to five cities, searched one at a time. Keep this page open. Results are saved after each city. Configured paid directories may charge for each search.</p>
