...
export async function searchGooglePlaces(params: { city: string; country: string; industry: string; apiKey: string; limit?: number }): Promise<RawBusiness[]> {
  ...
  const response = await fetch('https://places.googleapis.com/v1/places:searchText', {
    ...
    headers: { 'Content-Type': 'application/json', 'X-Goog-Api-Key': apiKey,
      'X-Goog-FieldMask': 'places.id,places.displayName,places.formattedAddress,places.internationalPhoneNumber,places.websiteUri,places.rating,places.userRatingCount,places.googleMapsUri,places.primaryTypeDisplayName,places.businessStatus' },
