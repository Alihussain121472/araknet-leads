      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-400">Manage all users and monitor platform usage.</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
            <h3 className="text-slate-400 font-medium">System-Wide Leads Discovered</h3>
            <p className="text-3xl font-bold text-cyan-400 mt-2">{stats.totalLeadsSystemWide}</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
            <h3 className="text-slate-400 font-medium">System-Wide Agent Runs</h3>
            ...
            <tbody className="divide-y divide-slate-800">
              {users.map(u => (
                <tr key={u.id} className="hover:bg-slate-800/50">
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${u.role === 'admin' ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-800 text-slate-300'}`}>
                      {u.role.toUpperCase()}
                    </span>
                  </td>
